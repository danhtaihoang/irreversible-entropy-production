...
!!! 2015.04.25: Tinh theo ca 2 cach M va S2 (order),S3 (disorder)
!!! 10.05.2015: Sua lai tinh theo epsilon
!!! 22.05.2015: Tinh P(M) song song voi viec update
!!! 27.05.2015: Luu y: eps o 1parameter.in chi la log(eps)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%! 
   PROGRAM main_BIO
   IMPLICIT NONE
   ...
   INTEGER (KIND=8) :: i,j,i1,i2,nx,ny,nz,natom,n_av,i_av,i_loop,n_equi,i_eps,n_eps,number
   INTEGER (KIND=8) :: time_2,time_3,time_5,time_6,time_7,compt,i_histo,nM
   REAL (KIND=8)    :: n_atom,E,E1,E_av,Jmp,rdn_s,rdn_mp,M1_av,M12_av,npositive,chi1,chi2,chi3
   REAL (KIND=8)    :: eps_min,eps_max,del_eps,eps,Mmin,Mmax,delM,M,M1,M2_av,E2_av,Cv
   REAL (KIND=8)    :: M3_av,M22_av,Jc
...
!!!=======================================================================================
   ...
   IF (n_eps==1) THEN
      del_eps=0.
   ELSE
      del_eps=(eps_max-eps_min)/real(n_eps-1)
   END IF
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   DO i_eps=1,n_eps
      ...
      eps=eps_min+del_eps*real(i_eps-1)
      
      !!!27.05.2015: Tinh epsilon tu log epsilon =========
      eps=-(10.**eps)
      !!!=================================================
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   READ(11, '(A50)')         tamp
   READ(11, '(A30,(F12.6))') tamp, Jc
   READ(11, '(A50)')         tamp
   READ(11, '(A50)')         tamp
   READ(11, '(A30,(F12.6))') tamp, eps_min
   READ(11, '(A30,(F12.6))') tamp, eps_max
   READ(11, '(A30,(I5))')    tamp, n_eps
   READ(11, '(A50)')         tamp
   READ(11, '(A50)')         tamp
   READ(11, '(A50)')         tamp
...
!!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   E_av=0. ; E2_av=0. ; M1_av=0. ; M2_av=0. ; M3_av=0. ; M12_av=0. ; M22_av=0.; npositive=0. 
   ...
   DO i_av=1,n_av
     ...
     M1_av=M1_av+abs(M)   !! <M> to estimated Tc
     M12_av=M12_av+M*M
           
     IF (M >= 0.) THEN  !! for ordered phase
         npositive=npositive+1.
         M2_av=M2_av+M
         M22_av=M22_av+M*M
     END IF
     
     M3_av=M3_av+M  !! for disordered phase
   ...
   M1_av=M1_av/n_atom/real(n_av)
   M2_av=M2_av/n_atom/npositive
   M3_av=M3_av/n_atom/real(n_av)
   
   M12_av=M12_av/n_atom/n_atom/real(n_av)
   M22_av=M22_av/n_atom/n_atom/npositive
       
   chi1=n_atom*(M12_av-M1_av**2.) 
   chi2=n_atom*(M22_av-M2_av**2.)
   chi3=n_atom*(M12_av-M3_av**2.)

   WRITE(line21,*)eps,E_av,Cv,M1_av,M2_av,M3_av,chi1,chi2,chi3
   ...
   DO i_histo=1,nM
      ...
      WRITE(line31,*)eps,M_histo(i_histo)/n_atom,P_histo(i_histo),-log(P_histo(i_histo))
